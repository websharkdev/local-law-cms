...
| **Транзакции** (бронирования, заявки, чаты, платежи) | PostgreSQL (Prisma) | Приложение + Strapi **Operations** (proxy UI, не копия в CMS) |
...
См. исходную спецификацию в репозитории приложения. Шаблоны документов (`document-template`) — контент CMS. Ops-данные (generated documents, translation requests, notary bookings, custom notary) управляются через plugin **Operations** и остаются в Prisma: см. [`docs/OPERATIONS.md`](OPERATIONS.md).
